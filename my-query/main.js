...
// src/main.ts
var main_exports = {};
__export(main_exports, { default: () => QueryPlugin });
...
var DEFAULT_SETTINGS = {
  ...
  showAI: true,
  primaryDictionary: "youdao",
  collectionSettings: {
    enabled: true,
    enableReview: true,
    aiNotePath: ".flashcards/words.json",
    prependMode: false,
    includeDefinition: true,
    includeTimestamp: true,
    flashcardPath: ".flashcards/words.json"
  },
  quickLinks: [
    { id: "youdao", name: "Youdao", url: "https://www.youdao.com/w/eng/{word}", icon: "\u{1F4D5}", enabled: true },
    { id: "cambridge", name: "Cambridge", url: "https://dictionary.cambridge.org/dictionary/english-chinese-simplified/{word}", icon: "\u{1F4D8}", enabled: true },
    { id: "merriam", name: "Merriam", url: "https://www.merriam-webster.com/dictionary/{word}", icon: "\u{1F4D7}", enabled: true },
    { id: "longman", name: "Longman", url: "https://www.ldoceonline.com/dictionary/{word}", icon: "\u{1F4D4}", enabled: true },
    { id: "zdic", name: "Zdic", url: "https://www.zdic.net/hans/{word}", icon: "\u{1F4D9}", enabled: true },
    { id: "google", name: "Google", url: "https://www.google.com/search?q={word}", icon: "\u{1F50D}", enabled: true },
    { id: "wikipedia", name: "Wikipedia", url: "https://en.wikipedia.org/wiki/{word}", icon: "\u{1F4D6}", enabled: true },
    { id: "eudic", name: "Eudic", url: "eudic://dict//{word}", icon: "\u{1F4F1}", enabled: true },
    { id: "youglish", name: "YouGlish", url: "https://youglish.com/pronounce/{word}/english", icon: "\u{1F3A5}", enabled: true },
    { id: "scholar", name: "Scholar", url: "https://scholar.google.com/scholar?q={word}", icon: "\u{1F393}", enabled: true },
    { id: "ludwig", name: "Ludwig", url: "https://ludwig.guru/s/{word}", icon: "\u{1F4AC}", enabled: true },
    { id: "justscience", name: "JustSci", url: "https://sci.justscience.cn/index.php?q={word}", icon: "\u{1F9EA}", enabled: true },
    { id: "cnki", name: "CNKI", url: "https://kns.cnki.net/kns8/defaultresult/index?kw={word}", icon: "\u{1F4DA}", enabled: true },
    { id: "chatgpt", name: "ChatGPT", url: "https://chatgpt.com/?q={word}", icon: "\u{1F916}", enabled: true },
    { id: "souyun", name: "Sou-Yun", url: "https://sou-yun.cn/CharInClause.aspx?char={word}", icon: "\u{1F4DA}", enabled: true }
  ],
  historySettings: {
    enabled: true,
    historyPath: ".flashcards/history.json"
  },
  reviewSettings: {
  ...
  aiServices: [
    {
      ...
      model: "gpt-3.5-turbo",
      prompt: '\u8BF7\u7B80\u6D01\u89E3\u91CA"{word}"\u8FD9\u4E2A\u8BCD\u7684\u542B\u4E49\uFF0C\u5305\u62EC\uFF1A1.\u5B9A\u4E49 2.\u4F8B\u53E5 3.\u7528\u6CD5\u8981\u70B9\u3002\u63A7\u5236\u5728150\u5B57\u4EE5\u5185\u3002'
    }
  ]
};
var WordLookupSettingTab = class extends import_obsidian.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.quickLinksSettingsOpen = false;
    this.plugin = plugin;
  }
  display() {
    const { containerEl } = this;
    containerEl.empty();
    containerEl.createEl("h2", { text: "\u663E\u793A\u6A21\u5F0F" });
    new import_obsidian.Setting(containerEl).setName("\u663E\u793A\u6A21\u5F0F").setDesc("\u9009\u62E9\u67E5\u8BCD\u7ED3\u679C\u7684\u663E\u793A\u65B9\u5F0F").addDropdown(
      (dropdown) => dropdown.addOption("panel", "\u53F3\u4FA7\u9762\u677F (iOS\u517C\u5BB9)").addOption("floating", "\u6709\u9053\u6D6E\u52A8\u6C14\u6CE1 (\u7B80\u660E)").setValue(this.plugin.settings.displayMode).onChange(async (value) => {
        this.plugin.settings.displayMode = value;
        await this.plugin.saveSettings();
      })
    );
    containerEl.createEl("h3", { text: "\u4E2D\u6587\u8BCD\u5178" });
    new import_obsidian.Setting(containerEl).setName("\u6C49\u5178 (Zdic)").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.chineseDictionaries.zdic).onChange(async (value) => {
        this.plugin.settings.chineseDictionaries.zdic = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u641C\u97F5 (Sou-Yun)").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.chineseDictionaries.souyun).onChange(async (value) => {
        this.plugin.settings.chineseDictionaries.souyun = value;
        await this.plugin.saveSettings();
      })
    );
    containerEl.createEl("h3", { text: "\u82F1\u6587\u8BCD\u5178" });
    new import_obsidian.Setting(containerEl).setName("\u6709\u9053 (Youdao)").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.englishDictionaries.youdao).onChange(async (value) => {
        this.plugin.settings.englishDictionaries.youdao = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u6717\u6587 (Longman)").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.englishDictionaries.longman).onChange(async (value) => {
        this.plugin.settings.englishDictionaries.longman = value;
        await this.plugin.saveSettings();
      })
    );
    containerEl.createEl("h2", { text: "\u57FA\u672C\u8BBE\u7F6E" });
    new import_obsidian.Setting(containerEl).setName("\u89E6\u53D1\u65B9\u5F0F").setDesc("\u9009\u62E9\u89E6\u53D1\u67E5\u8BCD\u7684\u529F\u80FD\u952E").addDropdown(
      (dropdown) => dropdown.addOption("altKey", "Alt + \u9009\u4E2D").addOption("ctrlKey", "Ctrl + \u9009\u4E2D").addOption("shiftKey", "Shift + \u9009\u4E2D").addOption("disable", "\u7981\u7528").setValue(this.plugin.settings.functionKey).onChange(async (value) => {
        this.plugin.settings.functionKey = value;
        await this.plugin.saveSettings();
      })
    );
    containerEl.createEl("h2", { text: "\u6536\u85CF\u8BBE\u7F6E" });
    new import_obsidian.Setting(containerEl).setName("\u542F\u7528\u6536\u85CF").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.collectionSettings.enabled).onChange(async (value) => {
        this.plugin.settings.collectionSettings.enabled = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u6536\u85CF\u6587\u4EF6\u8DEF\u5F84").addText(
      (text3) => text3.setPlaceholder("vocabulary/words.md").setValue(this.plugin.settings.collectionSettings.filePath).onChange(async (value) => {
        this.plugin.settings.collectionSettings.filePath = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("AI \u7B14\u8BB0\u8DEF\u5F84").setDesc("AI \u6536\u85CF\u5185\u5BB9\u4FDD\u5B58\u7684\u6587\u4EF6\u8DEF\u5F84").addText(
      (text3) => text3.setPlaceholder("sources/vocabulary/gpt-notes.md").setValue(this.plugin.settings.collectionSettings.aiNotePath).onChange(async (value) => {
        this.plugin.settings.collectionSettings.aiNotePath = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Flashcard JSON \u8DEF\u5F84").setDesc("AI \u7ED3\u679C\u4FDD\u5B58\u4E3A Flashcard \u7684 JSON \u6587\u4EF6\u8DEF\u5F84").addText(
      (text3) => text3.setPlaceholder(".flashcards/words.json").setValue(this.plugin.settings.collectionSettings.flashcardPath).onChange(async (value) => {
        this.plugin.settings.collectionSettings.flashcardPath = value;
        await this.plugin.saveSettings();
      })
    );
    containerEl.createEl("h2", { text: "\u5355\u8BCD\u590D\u4E60" });
    new import_obsidian.Setting(containerEl).setName("\u6BCF\u65E5\u65B0\u8BCD\u4E0A\u9650").setDesc("\u6BCF\u5929\u6700\u591A\u5B66\u4E60\u591A\u5C11\u4E2A\u65B0\u5355\u8BCD").addText(
      (text3) => text3.setValue(String(this.plugin.settings.reviewSettings.maxNewWordsPerDay)).onChange(async (value) => {
        const num = parseInt(value);
        if (!isNaN(num)) {
          this.plugin.settings.reviewSettings.maxNewWordsPerDay = num;
          await this.plugin.saveSettings();
        }
      })
    );
    new import_obsidian.Setting(containerEl).setName("\u6BCF\u65E5\u590D\u4E60\u4E0A\u9650").setDesc("\u6BCF\u5929\u6700\u591A\u590D\u4E60\u591A\u5C11\u4E2A\u5355\u8BCD").addText(
      (text3) => text3.setValue(String(this.plugin.settings.reviewSettings.maxReviewsPerDay)).onChange(async (value) => {
        const num = parseInt(value);
        if (!isNaN(num)) {
          this.plugin.settings.reviewSettings.maxReviewsPerDay = num;
          await this.plugin.saveSettings();
        }
      })
    );
    const algoDetails = containerEl.createEl("details");
    algoDetails.createEl("summary", { text: "\u9AD8\u7EA7\u7B97\u6CD5\u8BBE\u7F6E (SRS)" });
    new import_obsidian.Setting(algoDetails).setName("Initial Ease Factor").setDesc("Default difficulty multiplier (Default: 2.5)").addText(
      (text3) => text3.setValue(String(this.plugin.settings.reviewSettings.initialEase)).onChange(async (val2) => {
        const num = parseFloat(val2);
        if (!isNaN(num)) {
          this.plugin.settings.reviewSettings.initialEase = num;
          await this.plugin.saveSettings();
        }
      })
    );
    new import_obsidian.Setting(containerEl).setName("Flashcard Width").setDesc("Width of the flashcard in pixels (Default: 800)").addSlider(
      (slider) => slider.setLimits(400, 1200, 50).setValue(this.plugin.settings.flashcardWidth).setDynamicTooltip().onChange(async (val2) => {
        this.plugin.settings.flashcardWidth = val2;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Flashcard Height").setDesc("Minimum height of the flashcard in pixels (Default: 400)").addSlider(
      (slider) => slider.setLimits(200, 800, 50).setValue(this.plugin.settings.flashcardHeight).setDynamicTooltip().onChange(async (val2) => {
        this.plugin.settings.flashcardHeight = val2;
        await this.plugin.saveSettings();
      })
    );
    const aiDetails = containerEl.createEl("details");
    if (this.plugin.settings.showAI) {
      aiDetails.setAttribute("open", "");
    }
    aiDetails.createEl("summary", { text: "AI \u8BBE\u7F6E (\u70B9\u51FB\u5C55\u5F00)" });
    new import_obsidian.Setting(aiDetails).setName("\u542F\u7528 AI \u89E3\u91CA").setDesc("\u4F7F\u7528 AI \u63D0\u4F9B\u989D\u5916\u7684\u5355\u8BCD\u89E3\u91CA").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.showAI).onChange(async (value) => {
        this.plugin.settings.showAI = value;
        await this.plugin.saveSettings();
      })
    );
    const aiServicesList = aiDetails.createDiv("ai-services-list");
    aiServicesList.style.marginTop = "10px";
    aiServicesList.style.borderLeft = "2px solid var(--background-modifier-border)";
    aiServicesList.style.paddingLeft = "10px";
    this.plugin.settings.aiServices.forEach((service, index2) => {
      const serviceDiv = aiServicesList.createDiv("ai-service-item");
      serviceDiv.style.marginBottom = "15px";
      serviceDiv.style.padding = "10px";
      serviceDiv.style.background = "var(--background-secondary)";
      serviceDiv.style.borderRadius = "5px";
      const headerDiv = serviceDiv.createDiv("ai-service-header");
      headerDiv.style.display = "flex";
      headerDiv.style.alignItems = "center";
      headerDiv.style.gap = "10px";
      headerDiv.style.marginBottom = "10px";
      new import_obsidian.Setting(headerDiv).addToggle(
        (t) => t.setValue(service.enabled).onChange(async (val2) => {
          if (val2) {
            this.plugin.settings.aiServices.forEach((s, i) => {
              if (i !== index2)
                s.enabled = false;
            });
          }
          this.plugin.settings.aiServices[index2].enabled = val2;
          await this.plugin.saveSettings();
          this.display();
        })
      );
      headerDiv.lastChild.querySelector(".setting-item-info").style.display = "none";
      headerDiv.lastChild.querySelector(".setting-item-control").style.justifyContent = "flex-start";
      ...
      nameInput.onchange = async () => {
        this.plugin.settings.aiServices[index2].name = nameInput.value;
        await this.plugin.saveSettings();
      };
      ...
      deleteBtn.onclick = async () => {
        if (confirm("\u786E\u5B9A\u8981\u5220\u9664\u8FD9\u4E2A AI \u670D\u52A1\u914D\u7F6E\u5417\uFF1F")) {
          this.plugin.settings.aiServices.splice(index2, 1);
          await this.plugin.saveSettings();
          this.display();
        }
      };
      headerDiv.appendChild(deleteBtn);
      new import_obsidian.Setting(serviceDiv).setName("\u63D0\u4F9B\u5546").addDropdown(
        (dropdown) => dropdown.addOption("openai", "OpenAI (ChatGPT)").addOption("anthropic", "Anthropic (Claude)").addOption("generic", "\u81EA\u5B9A\u4E49 API").setValue(service.provider).onChange(async (val2) => {
          this.plugin.settings.aiServices[index2].provider = val2;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian.Setting(serviceDiv).setName("API Key").addText(
        (text3) => text3.setPlaceholder("sk-...").setValue(service.apiKey).onChange(async (val2) => {
          this.plugin.settings.aiServices[index2].apiKey = val2;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian.Setting(serviceDiv).setName("Base URL").addText(
        (text3) => text3.setPlaceholder("https://api.openai.com/v1").setValue(service.baseURL).onChange(async (val2) => {
          this.plugin.settings.aiServices[index2].baseURL = val2;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian.Setting(serviceDiv).setName("\u6A21\u578B").addText(
        (text3) => text3.setPlaceholder("gpt-3.5-turbo").setValue(service.model).onChange(async (val2) => {
          this.plugin.settings.aiServices[index2].model = val2;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian.Setting(serviceDiv).setName("Prompt \u6A21\u677F").setDesc("\u4F7F\u7528 {word} \u8868\u793A\u67E5\u8BE2\u7684\u5355\u8BCD").addTextArea(
        (text3) => text3.setPlaceholder("\u8BF7\u89E3\u91CA {word}").setValue(service.prompt).onChange(async (val2) => {
          this.plugin.settings.aiServices[index2].prompt = val2;
          await this.plugin.saveSettings();
        })
      );
      const exportDetails = serviceDiv.createEl("details");
      exportDetails.style.marginTop = "10px";
      exportDetails.createEl("summary", { text: "\u5BFC\u51FA\u8BBE\u7F6E (\u53EF\u9009)" });
      new import_obsidian.Setting(exportDetails).setName("\u5BFC\u51FA\u6587\u4EF6\u8DEF\u5F84").setDesc("\u8986\u76D6\u9ED8\u8BA4\u7684 AI \u7B14\u8BB0\u8DEF\u5F84").addText(
        (text3) => text3.setPlaceholder("sources/ai/gpt-custom.md").setValue(service.exportPath || "").onChange(async (val2) => {
          this.plugin.settings.aiServices[index2].exportPath = val2;
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian.Setting(exportDetails).setName("\u5BFC\u51FA\u683C\u5F0F\u6A21\u677F").setDesc("\u652F\u6301 {word} \u548C {content} \u5360\u4F4D\u7B26").addTextArea(
        (text3) => text3.setPlaceholder("## {word}\n\n{content}\n\n---\n").setValue(service.exportTemplate || "").onChange(async (val2) => {
          this.plugin.settings.aiServices[index2].exportTemplate = val2;
          await this.plugin.saveSettings();
        })
      );
    });
    new import_obsidian.Setting(aiDetails).addButton(
      (btn) => btn.setButtonText("\u2795 \u6DFB\u52A0 AI \u670D\u52A1").onClick(async () => {
    ...
    const quickLinksDetails = containerEl.createEl("details");
    if (this.quickLinksSettingsOpen) {
      quickLinksDetails.setAttribute("open", "");
    }
    quickLinksDetails.createEl("summary", { text: "\u{1F517} \u5FEB\u901F\u94FE\u63A5\u8BBE\u7F6E (\u70B9\u51FB\u5C55\u5F00)" });
    quickLinksDetails.addEventListener("toggle", () => {
      this.quickLinksSettingsOpen = quickLinksDetails.open;
    });
    quickLinksDetails.createEl("p", { text: "\u81EA\u5B9A\u4E49\u5FEB\u901F\u94FE\u63A5\u3002\u5728URL\u4E2D\u4F7F\u7528 {word} \u4F5C\u4E3A\u5355\u8BCD\u5360\u4F4D\u7B26\u3002" });
    const quickLinksList = quickLinksDetails.createDiv("quick-links-settings-list");
    this.plugin.settings.quickLinks.forEach((link, index2) => {
      const linkDiv = quickLinksList.createDiv("quick-link-setting-item");
      linkDiv.style.display = "flex";
      linkDiv.style.alignItems = "center";
      linkDiv.style.marginBottom = "10px";
      linkDiv.style.gap = "10px";
      const toggle = new import_obsidian.Setting(linkDiv).addToggle(
        (t) => t.setValue(link.enabled).onChange(async (val2) => {
          this.plugin.settings.quickLinks[index2].enabled = val2;
          await this.plugin.saveSettings();
        })
      );
      toggle.controlEl.style.margin = "0";
      const nameInput = document.createElement("input");
      nameInput.type = "text";
      ...
      nameInput.onchange = async () => {
        this.plugin.settings.quickLinks[index2].name = nameInput.value;
        await this.plugin.saveSettings();
      };
      linkDiv.appendChild(nameInput);
      const urlInput = document.createElement("input");
      urlInput.type = "text";
      urlInput.value = link.url;
      urlInput.placeholder = "URL (\u4F7F\u7528 {word})";
      urlInput.style.flex = "1";
      urlInput.onchange = async () => {
        this.plugin.settings.quickLinks[index2].url = urlInput.value;
        await this.plugin.saveSettings();
      };
      ...
      deleteBtn.title = "\u5220\u9664";
      deleteBtn.onclick = async () => {
        this.plugin.settings.quickLinks.splice(index2, 1);
        await this.plugin.saveSettings();
        this.display();
      };
      linkDiv.appendChild(deleteBtn);
    });
    const addBtnContainer = quickLinksDetails.createDiv();
    const addBtn = new import_obsidian.Setting(addBtnContainer).addButton(
      (btn) => btn.setButtonText("\u2795 \u6DFB\u52A0\u65B0\u94FE\u63A5").onClick(async () => {
        this.plugin.settings.quickLinks.push({
          id: Date.now().toString(),
          name: "New Link",
          url: "https://example.com/search?q={word}",
          icon: "\u{1F517}",
          enabled: true
        });
        await this.plugin.saveSettings();
    ...
    const resetBtn = new import_obsidian.Setting(addBtnContainer).addButton(
      (btn) => btn.setButtonText("\u21BA \u91CD\u7F6E\u4E3A\u9ED8\u8BA4").setWarning().onClick(async () => {
        if (confirm("\u786E\u5B9A\u8981\u91CD\u7F6E\u6240\u6709\u5FEB\u901F\u94FE\u63A5\u4E3A\u9ED8\u8BA4\u503C\u5417\uFF1F\u8FD9\u5C06\u5220\u9664\u6240\u6709\u81EA\u5B9A\u4E49\u94FE\u63A5\u3002")) {
          this.plugin.settings.quickLinks = JSON.parse(JSON.stringify(DEFAULT_SETTINGS.quickLinks));
          await this.plugin.saveSettings();
          this.display();
        }
      })
...
// src/dictionary/youdao.ts
var import_obsidian2 = require("obsidian");
var YoudaoDictionary = class {
  getName() {
    return "Youdao";
  }
  /**
   * Get concise definition for floating result using suggest API (simpler/faster)
   */
  async getConcise(word) {
    try {
      ...
      if (data2 && data2.data && data2.data.entries && data2.data.entries.length > 0) {
        const entry = data2.data.entries[0];
...
${explain}`;
      ...
      console.error("Youdao concise lookup failed:", error);
      return "";
    }
  }
  async lookup(word) {
    try {
      ...
      const response = await (0, import_obsidian2.requestUrl)({ url });
      const data2 = response.json;
      const detailUrl = `https://dict.youdao.com/w/eng/${encodeURIComponent(word)}/`;
      const detailResponse = await (0, import_obsidian2.requestUrl)({ url: detailUrl });
      const html3 = detailResponse.text;
      const result = {
        word
      };
      const phoneticMatch = html3.match(/class="phonetic">([^<]+)<\/span>/);
      if (phoneticMatch) {
        result.phonetic = phoneticMatch[1].trim();
      }
      const usMatch = html3.match(/美\s*\[([^\]]+)\]/);
      if (usMatch) {
        result.phoneticUS = usMatch[1];
      }
      const ukMatch = html3.match(/英\s*\[([^\]]+)\]/);
      if (ukMatch) {
        result.phoneticUK = ukMatch[1];
      }
      const transMatch = html3.match(/<div class="trans-container">[\s\S]*?<ul>([\s\S]*?)<\/ul>/);
      if (transMatch) {
        const translations = [];
        const liMatches = transMatch[1].matchAll(/<li>([^<]+)<\/li>/g);
        for (const match of liMatches) {
          translations.push(match[1].trim());
        }
        result.basicTranslation = translations;
      }
      const webTransMatch = html3.match(/<div id="webTrans">[\s\S]*?<div class="wt-container">([\s\S]*?)<\/div>/);
      if (webTransMatch) {
        const webTranslations = [];
        const webItems = webTransMatch[1].matchAll(/<div class="title">[\s\S]*?<a[^>]*>([^<]+)<\/a>[\s\S]*?<\/div>[\s\S]*?<p class="contentTitle">[\s\S]*?<span>([^<]+)<\/span>/g);
        for (const match of webItems) {
          webTranslations.push({
            key: match[1].trim(),
            value: [match[2].trim()]
          });
        }
        if (webTranslations.length > 0) {
          result.webTranslation = webTranslations;
        }
      }
      return result;
    } catch (error) {
      console.error("Youdao lookup failed:", error);
      throw error;
    }
  }
  /**
   * Format search result as HTML string
   * @param result Dictionary lookup result  
   * @param word The word being looked up
   * @param settings Plugin settings
   * @returns Formatted HTML string
   */
  formatResult(result, word, settings) {
    const hasContent = result.basicTranslation && result.basicTranslation.length > 0 || result.webTranslation && result.webTranslation.length > 0 || result.phonetic || result.phoneticUS || result.phoneticUK;
    if (!hasContent) {
      return "";
    }
    let html3 = `<div class="word-lookup-result">`;
    html3 += `<div class="dict-source">\u{1F4D8} Youdao Dictionary</div>`;
    html3 += `<div class="word-title">${result.word}</div>`;
    if (result.phoneticUS || result.phoneticUK || result.phonetic) {
      html3 += `<div class="word-phonetic">`;
      if (result.phoneticUS) {
        html3 += `<span class="phonetic-us">\u7F8E [${result.phoneticUS}]</span> `;
      }
      if (result.phoneticUK) {
        html3 += `<span class="phonetic-uk">\u82F1 [${result.phoneticUK}]</span>`;
      }
      if (result.phonetic && !result.phoneticUS && !result.phoneticUK) {
        html3 += `<span>${result.phonetic}</span>`;
      }
      const audioUrl = `https://dict.youdao.com/dictvoice?type=1&audio=${encodeURIComponent(result.word)}`;
      html3 += ` <a href="${audioUrl}" class="speaker" title="Play pronunciation">\u{1F50A}</a>`;
      html3 += `</div>`;
    }
    if (result.basicTranslation && result.basicTranslation.length > 0) {
      html3 += `<div class="word-basic-trans">`;
      html3 += `<div class="trans-title">\u57FA\u672C\u91CA\u4E49</div>`;
      html3 += `<ul>`;
      for (const trans of result.basicTranslation) {
        html3 += `<li>${trans}</li>`;
      }
      html3 += `</ul>`;
      html3 += `</div>`;
    }
    if (result.webTranslation && result.webTranslation.length > 0) {
      html3 += `<div class="word-web-trans">`;
      html3 += `<div class="trans-title">\u7F51\u7EDC\u91CA\u4E49</div>`;
      html3 += `<ul>`;
      for (const trans of result.webTranslation) {
        html3 += `<li><strong>${trans.key}</strong>: ${trans.value.join("; ")}</li>`;
      }
      html3 += `</ul>`;
      html3 += `</div>`;
    }
    html3 += `<div class="word-web-trans">`;
    html3 += `<a href="https://www.youdao.com/w/eng/${encodeURIComponent(word)}" target="_blank" class="dict-link">\u{1F4D5} \u67E5\u770B\u6709\u9053\u5728\u7EBF\u8BCD\u5178 \u2192</a>`;
    html3 += `</div>`;
    html3 += `</div>`;
    return html3;
  }
  /**
   * Render quick links based on user settings
   */
  renderQuickLinks(word, settings) {
    if (!Array.isArray(settings))
      return "";
    const enabledLinks = settings.filter((link) => link.enabled);
    if (enabledLinks.length === 0) {
      return "";
    }
    let html3 = '<div class="quick-links">';
    html3 += '<div class="quick-links-buttons" style="margin-top: 10px; display: flex; flex-wrap: wrap; gap: 8px;">';
    enabledLinks.forEach((link) => {
      const finalUrl = link.url.replace("{word}", encodeURIComponent(word));
      html3 += `<a href="${finalUrl}" class="quick-link-btn" target="_blank" style="text-decoration: none; color: var(--text-muted); font-size: 12px; padding: 2px 6px; background: var(--background-secondary); border-radius: 4px;">${link.icon || "\u{1F517}"} ${link.name}</a>`;
    });
    html3 += "</div></div>";
    return html3;
  }
...
// src/dictionary/longman-scraper.ts
...
// src/dictionary/longman-scraper.ts
async function scrapeLongman(word) {
  const result = {
    word,
    definitions: []
  };
  try {
    ...
    const $2 = load(html3);
    const pronText = $2(".PronCodes").first().text();
    if (pronText) {
      result.pronunciation = pronText.trim();
    }
    $2(".Sense").each((i, el) => {
      if (result.definitions.length >= 5)
        return false;
      const sense = $2(el);
      const defText = sense.find(".DEF").first().text().trim();
      if (!defText)
        return;
      const examples = [];
      sense.find(".EXAMPLE").each((j, exEl) => {
        if (examples.length < 3) {
          examples.push($2(exEl).text().trim());
        }
      });
      const grammar = sense.find(".GRAM").first().text().trim() || void 0;
    ...
    console.error("Longman scraping failed:", error);
  }
  return result;
}

// src/dictionary/longman.ts
var LongmanDictionary = class {
  getName() {
    return "Longman";
  }
  async lookup(word) {
    try {
      const data2 = await scrapeLongman(word);
      const result = {
        word
      };
      if (data2.pronunciation) {
        result.phonetic = data2.pronunciation;
      }
      const translations = [];
      if (data2.definitions && data2.definitions.length > 0) {
        data2.definitions.forEach((def, index2) => {
          translations.push(`${index2 + 1}. ${def.definition}`);
          if (def.examples && def.examples.length > 0) {
            def.examples.forEach((ex) => {
              translations.push(`   \u2022 ${ex}`);
            });
          }
        });
      }
      ...
      console.error("Longman lookup failed:", error);
      return {
        word,
        basicTranslation: ["\u67E5\u8BE2\u5931\u8D25\uFF0C\u8BF7\u4F7F\u7528\u5FEB\u901F\u94FE\u63A5\u8BBF\u95EE Longman \u5728\u7EBF\u8BCD\u5178"]
      };
    }
  }
  formatResult(result, word, settings) {
    ...
    let html3 = `<div class="word-lookup-result">`;
    html3 += `<div class="dict-source">\u{1F4D4} Longman Dictionary</div>`;
    html3 += `<div class="word-title">${result.word}</div>`;
    if (result.phonetic) {
      html3 += `<div class="word-phonetic">`;
      html3 += `<span>${result.phonetic}</span>`;
    ...
    if (result.basicTranslation && result.basicTranslation.length > 0) {
      html3 += `<div class="word-basic-trans">`;
      html3 += `<div class="trans-title">Definitions & Examples</div>`;
      html3 += `<ul style="list-style: none; padding-left: 0;">`;
      for (const trans of result.basicTranslation) {
        if (/^\d+\./.test(trans)) {
          html3 += `<li style="margin-top: 8px; font-weight: 500;">${trans}</li>`;
        } else if (trans.trim().startsWith("\u2022")) {
          html3 += `<li style="margin-left: 20px; font-style: italic; color: #666;">${trans}</li>`;
        } else {
          html3 += `<li>${trans}</li>`;
        }
      }
    ...
    html3 += `<div class="word-web-trans">`;
    html3 += `<a href="https://www.ldoceonline.com/dictionary/${encodeURIComponent(word)}" target="_blank" class="dict-link">\u{1F4D4} \u67E5\u770B\u6717\u6587\u5728\u7EBF\u8BCD\u5178 \u2192</a>`;
    html3 += `</div>`;
    html3 += `</div>`;
    return html3;
  }
  renderQuickLinks(word, quickLinks) {
    const links = [
      { name: "Youdao", url: `https://www.youdao.com/w/eng/${encodeURIComponent(word)}`, enabled: quickLinks.youdao, icon: "\u{1F4D5}" },
      { name: "Cambridge", url: `https://dictionary.cambridge.org/dictionary/english-chinese-simplified/${encodeURIComponent(word)}`, enabled: quickLinks.cambridge, icon: "\u{1F4D8}" },
      { name: "Merriam", url: `https://www.merriam-webster.com/dictionary/${encodeURIComponent(word)}`, enabled: quickLinks.merriam, icon: "\u{1F4D7}" },
      { name: "Longman", url: `https://www.ldoceonline.com/dictionary/${encodeURIComponent(word)}`, enabled: quickLinks.longman, icon: "\u{1F4D4}" },
      { name: "Zdic", url: `https://www.zdic.net/hans/${encodeURIComponent(word)}`, enabled: quickLinks.zdic, icon: "\u{1F4D9}" },
      { name: "Google", url: `https://www.google.com/search?q=${encodeURIComponent(word)}`, enabled: quickLinks.google, icon: "\u{1F50D}" },
      { name: "Wikipedia", url: `https://en.wikipedia.org/wiki/${encodeURIComponent(word)}`, enabled: quickLinks.wikipedia, icon: "\u{1F4D6}" },
      { name: "Eudic", url: `eudic://dict//${encodeURIComponent(word)}`, enabled: quickLinks.eudic, icon: "\u{1F4F1}" },
      { name: "YouGlish", url: `https://youglish.com/pronounce/${encodeURIComponent(word)}/english`, enabled: quickLinks.youglish, icon: "\u{1F3A5}" },
      { name: "Scholar", url: `https://scholar.google.com/scholar?q=${encodeURIComponent(word)}`, enabled: quickLinks.scholar, icon: "\u{1F393}" },
      { name: "Ludwig", url: `https://ludwig.guru/s/${encodeURIComponent(word)}`, enabled: quickLinks.ludwig, icon: "\u{1F4AC}" },
      { name: "JustSci", url: `https://sci.justscience.cn/index.php?q=${encodeURIComponent(word)}`, enabled: quickLinks.justscience, icon: "\u{1F9EA}" },
      { name: "CNKI", url: `https://kns.cnki.net/kns8/defaultresult/index?kw=${encodeURIComponent(word)}`, enabled: quickLinks.cnki, icon: "\u{1F4DA}" },
      { name: "ChatGPT", url: `https://chatgpt.com/?q=${encodeURIComponent(word)}`, enabled: quickLinks.chatgpt, icon: "\u{1F916}" },
      { name: "Sou-Yun", url: `https://sou-yun.cn/CharInClause.aspx?char=${encodeURIComponent(word)}`, enabled: quickLinks.souyun, icon: "\u{1F4DA}" }
    ];
    const enabledLinks = links.filter((l) => l.enabled);
    if (enabledLinks.length === 0)
      return "";
    let html3 = '<div class="quick-links">';
    html3 += `<div class="quick-links-header" onclick="this.parentElement.classList.toggle('collapsed')">`;
    html3 += '<span class="quick-links-label">\u{1F517} \u5FEB\u901F\u94FE\u63A5</span>';
    html3 += '<span class="quick-links-toggle">\u25BC</span>';
    html3 += "</div>";
    html3 += '<div class="quick-links-buttons">';
    enabledLinks.forEach((link) => {
      html3 += `<a href="${link.url}" class="quick-link-btn" target="_blank">${link.icon} ${link.name}</a>`;
    });
    html3 += "</div></div>";
    return html3;
  }
...
// src/dictionary/zdic-scraper.ts
var import_obsidian4 = require("obsidian");
async function scrapeZdic(word) {
  const result = {
    word,
    definitions: [],
    idioms: []
  ...
  try {
    ...
    const pinyinMatch = html3.match(/class="dicpy"[^>]*>([^<]+)</);
    if (pinyinMatch) {
      result.pinyin = pinyinMatch[1].replace(/\s+/g, " ").trim();
    }
    const jbjsMatch = html3.match(/class="jbjs"[\s\S]*?class="jnr"[\s\S]*?>([\s\S]*?)<\/div>/) || html3.match(/class="jnr"[^>]*>([\s\S]*?)<\/div>/);
    if (jbjsMatch) {
      const content = jbjsMatch[1];
      const olMatch = content.match(/<ol>([\s\S]*?)<\/ol>/);
      if (olMatch) {
        const liMatches = olMatch[1].matchAll(/<li>([\s\S]*?)<\/li>/g);
        for (const match of liMatches) {
          const cleanText = stripTags(match[1]);
          if (cleanText) {
            result.definitions.push({ text: cleanText });
          }
        }
      }
      if (result.definitions.length === 0) {
    ...
    if (result.definitions.length === 0) {
      const olMatch = html3.match(/<div class="content definitions">([\s\S]*?)<\/div>/) || html3.match(/<ol>([\s\S]*?)<\/ol>/);
      if (olMatch) {
        const liMatches = olMatch[1].matchAll(/<li>([\s\S]*?)<\/li>/g);
        for (const match of liMatches) {
          const cleanText = stripTags(match[1]);
    ...
    const idiomMatches = html3.matchAll(/<a[^>]+href="[^"]*chengyu[^"]*"[^>]*>([^<]+)<\/a>/g);
    const idiomSet = /* @__PURE__ */ new Set();
    for (const match of idiomMatches) {
      const idiom = match[1].trim();
      if (idiom && idiom.length >= 2 && idiom.length <= 8 && !idiom.includes("\u66F4\u591A")) {
        idiomSet.add(idiom);
        if (idiomSet.size >= 8)
          break;
      }
    }
    result.idioms = Array.from(idiomSet);
    const variantMatch = html3.match(/异体字[：:]\s*([^<\n]{2,50})/);
    if (variantMatch) {
      result.variants = variantMatch[1].trim();
    }
  } catch (error) {
    console.error("Zdic scraping failed:", error);
  }
  return result;
}

// src/dictionary/zdic.ts
var ZdicDictionary = class {
  getName() {
    return "Zdic";
  }
  async lookup(word) {
    try {
      const data2 = await scrapeZdic(word);
      const result = {
        word
      };
      if (data2.pinyin) {
        result.phonetic = data2.pinyin;
      }
      const translations = [];
      if (data2.definitions && data2.definitions.length > 0) {
        translations.push("\u{1F4D6} \u91CA\u4E49\uFF1A");
        data2.definitions.forEach((def, index2) => {
          const prefix = def.category ? `\u3010${def.category}\u3011` : `${index2 + 1}.`;
          translations.push(`${prefix} ${def.text}`);
        });
      }
      if (data2.idioms && data2.idioms.length > 0) {
        translations.push("");
        translations.push("\u{1F524} \u76F8\u5173\u8BCD\u7EC4\uFF1A");
        const groupSize = 4;
        for (let i = 0; i < data2.idioms.length; i += groupSize) {
          const group = data2.idioms.slice(i, i + groupSize);
          translations.push(group.join("  \u2022  "));
        }
      }
      if (data2.variants) {
        translations.push("");
        translations.push(`\u270F\uFE0F \u5F02\u4F53\u5B57\uFF1A${data2.variants}`);
      }
      if (translations.length === 0) {
      }
      result.basicTranslation = translations;
      ...
      console.error("Zdic lookup failed:", error);
      return {
        word,
        basicTranslation: ["\u67E5\u8BE2\u5931\u8D25\uFF0C\u8BF7\u4F7F\u7528\u5FEB\u901F\u94FE\u63A5\u8BBF\u95EE\u6C49\u5178"]
      };
    }
  }
  formatResult(result, word, settings) {
    ...
    let html3 = `<div class="word-lookup-result">`;
    html3 += `<div class="dict-source">\u{1F4D9} \u6C49\u5178 (Zdic)</div>`;
    html3 += `<div class="word-title">${result.word}</div>`;
    if (result.phonetic) {
      html3 += `<div class="word-phonetic">`;
      html3 += `<span class="pinyin">${result.phonetic}</span>`;
      html3 += `</div>`;
    }
    if (result.basicTranslation && result.basicTranslation.length > 0) {
      html3 += `<div class="word-basic-trans">`;
      html3 += `<ul class="no-bullet" style="list-style: none !important; padding-left: 0;">`;
      for (const trans of result.basicTranslation) {
        if (trans.includes("\u{1F4D6}") || trans.includes("\u{1F524}") || trans.includes("\u270F\uFE0F")) {
          html3 += `</ul><div class="trans-title" style="margin-top: 10px;">${trans}</div><ul style="list-style: none; padding-left: 0;">`;
        } else if (trans.trim() === "") {
          continue;
        } else if (trans.includes("\u3010")) {
          html3 += `<li style="margin: 4px 0;"><strong>${trans}</strong></li>`;
        } else if (/^\d+\./.test(trans)) {
          html3 += `<li style="margin: 4px 0;">${trans}</li>`;
        } else if (trans.includes("\u2022")) {
          html3 += `<li style="margin: 2px 0; color: #555;">${trans}</li>`;
        } else {
          html3 += `<li style="margin: 4px 0;">${trans}</li>`;
        }
      }
    ...
    html3 += `<div class="word-web-trans">`;
    html3 += `<a href="https://www.zdic.net/hans/${encodeURIComponent(result.word)}" target="_blank" class="dict-link">\u67E5\u770B\u5B8C\u6574\u8BCD\u6761 \u2192</a>`;
    html3 += `</div>`;
    html3 += `</div>`;
    return html3;
  }
...
// src/dictionary/souyun-scraper.ts
var import_obsidian5 = require("obsidian");
var API_BASE = "https://api.sou-yun.cn/api/";
async function scrapeSouYun(char) {
  const result = {
    words: [],
    poems: []
  };
  try {
    const wordUrl = `${API_BASE}Word?id=${encodeURIComponent(char)}`;
    const wordResponse = await (0, import_obsidian5.requestUrl)({ url: wordUrl });
    const wordData = wordResponse.json;
    if (wordData.Words && Array.isArray(wordData.Words)) {
      result.words = wordData.Words.slice(0, 5).map((w) => ({
        text: w.Text || "",
        explanation: stripHtmlTags(w.Html || "")
      }));
    }
    const poemUrl = `${API_BASE}Poem?key=${encodeURIComponent(char)}&pageNo=0`;
    const poemResponse = await (0, import_obsidian5.requestUrl)({ url: poemUrl });
    const poemData = poemResponse.json;
    if (poemData.Poems && poemData.Poems.Htmls && Array.isArray(poemData.Poems.Htmls)) {
      result.poems = poemData.Poems.Htmls.slice(0, 5).map(
        (html3) => parsePoem(html3)
      ).filter((p) => p.title && p.content);
    }
  } catch (error) {
    console.error("Sou-Yun scraping failed:", error);
  }
  return result;
}
function parsePoem(html3) {
  const titleMatch = html3.match(/class='poemCommentLink'[^>]*>([^<]+)<\/a>/);
  const title = titleMatch ? titleMatch[1] : "";
  const authorMatch = html3.match(/javascript: ShowPoemAuthorProfile[^>]*>([^<]+)<\/a>/);
  const author = authorMatch ? authorMatch[1] : "";
  const dynastyMatch = html3.match(/class='inlineComment1'>([^·]+)·/);
  const dynasty = dynastyMatch ? dynastyMatch[1].trim() : "";
  const contentMatches = html3.matchAll(/class='poemSentence'[^>]*>([^<]+)</g);
  const sentences = [];
...
function stripHtmlTags(html3) {
  let text3 = html3.replace(/<[^>]+>/g, " ");
...
// src/dictionary/souyun.ts
var SouYunDictionary = class {
  getName() {
    return "Sou-Yun";
  }
  async lookup(word) {
    try {
      const data2 = await scrapeSouYun(word);
      const translations = [];
      if (data2.words && data2.words.length > 0) {
        translations.push("\u{1F4DA} \u76F8\u5173\u8BCD\u6C47\uFF1A");
        data2.words.forEach((w) => {
          if (w.text && w.explanation) {
            translations.push(`${w.text}: ${w.explanation}`);
          }
        });
      }
      if (data2.poems && data2.poems.length > 0) {
        translations.push("");
        translations.push("\u{1F4D6} \u8BD7\u8BCD\u7528\u4F8B\uFF1A");
        data2.poems.forEach((p) => {
          if (p.title && p.content) {
            const authorInfo = p.dynasty && p.author ? `${p.dynasty} \xB7 ${p.author}` : p.author || "";
            translations.push(`\u300A${p.title}\u300B${authorInfo ? ` - ${authorInfo}` : ""}`);
            const lines = p.content.split("\n").filter((l) => l.trim());
            if (lines.length > 0) {
              translations.push(`  ${lines[0]}`);
              if (lines.length > 1) {
                translations.push(`  ${lines[1]}`);
              }
            }
          }
        });
      ...
      return {
        word,
        basicTranslation: translations
      };
    } catch (error) {
      console.error("Sou-Yun lookup failed:", error);
      return {
        word,
        basicTranslation: [
          "\u641C\u97F5\u7F51\u662F\u53E4\u5178\u8BD7\u8BCD\u6587\u5B66\u641C\u7D22\u5F15\u64CE",
  ...
  formatResult(result, word, settings) {
    ...
    let html3 = `<div class="word-lookup-result">`;
    html3 += `<div class="dict-source">\u{1F4DA} \u641C\u97F5\u7F51 (Sou-Yun)</div>`;
    html3 += `<div class="word-title">${result.word}</div>`;
    if (result.basicTranslation && result.basicTranslation.length > 0) {
      html3 += `<div class="word-basic-trans">`;
      html3 += `<ul>`;
      for (const trans of result.basicTranslation) {
        if (trans.includes("\u{1F4DA}") || trans.includes("\u{1F4D6}")) {
          html3 += `</ul><div class="trans-title">${trans}</div><ul>`;
        } else if (trans.trim() === "") {
          continue;
        } else {
          html3 += `<li>${trans}</li>`;
...
// src/dictionary/factory.ts
var DictionaryFactory = class {
  static create(type) {
    switch (type) {
      case "youdao":
        return new YoudaoDictionary();
      case "longman":
        return new LongmanDictionary();
      case "zdic":
        return new ZdicDictionary();
      case "souyun":
        return new SouYunDictionary();
      default:
        return new YoudaoDictionary();
    }
  }
...
var WordCollection = class {
  ...
  async saveWord(result) {
    if (!this.settings.enabled) {
      new import_obsidian6.Notice("Word collection is disabled in settings");
      return;
    }
    if (!this.settings.filePath) {
      new import_obsidian6.Notice("Please set collection file path in settings");
      return;
    }
    try {
      const file = await this.ensureFile();
      const existing = await this.app.vault.read(file);
      const wordEscaped = result.word.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const regex = new RegExp(`^-\\s*${wordEscaped}[:\uFF1A]`, "m");
      if (regex.test(existing)) {
        new import_obsidian6.Notice(`"${result.word}" \u5DF2\u5B58\u5728\u4E8E\u751F\u8BCD\u672C\u4E2D`);
        return;
      }
      const content = this.formatWord(result);
      let newContent = "";
      ...
      new import_obsidian6.Notice(`\u2705 "${result.word}" \u5DF2\u4FDD\u5B58`);
    } catch (error) {
      console.error("Failed to save word:", error);
      new import_obsidian6.Notice("\u274C Failed to save word");
    }
  }
  async ensureFile() {
    const path = this.settings.filePath;
    let file = this.app.vault.getAbstractFileByPath(path);
    if (!file) {
      file = await this.app.vault.create(path, "# Word Collection\n\n");
    }
  ...
  formatWord(result) {
    var _a5;
    const definition = ((_a5 = result.basicTranslation) == null ? void 0 : _a5.join("; ")) || "No definition";
    return `- ${result.word}: ${definition}`;
  }
};
...
var FlashLookupComponent = class {
  constructor(app) {
  ...
  mount(container) {
    ...
    container.empty();
    container.addClass("flash-lookup-container");
    const searchContainer = container.createDiv({ cls: "flash-search-container" });
    this.searchInput = searchContainer.createEl("input", {
      cls: "flash-search-input",
      type: "text",
      placeholder: "\u8F93\u5165\u5355\u8BCD\u67E5\u8BE2..."
    });
    ...
    this.flashContentEl = container.createDiv({ cls: "flash-content" });
    this.flashContentEl.innerHTML = '<div class="flash-placeholder">\u9009\u4E2D\u6587\u5B57\u540E\u6309Alt\u952E\u67E5\u8BCD<br>\u6216\u5728\u4E0A\u65B9\u8F93\u5165\u67E5\u8BE2</div>';
  }
  setContent(html3) {
    if (this.flashContentEl) {
      this.flashContentEl.innerHTML = html3;
      this.flashContentEl.querySelectorAll(".quick-links-header").forEach((header) => {
        header.addEventListener("click", () => {
          var _a5;
          (_a5 = header.parentElement) == null ? void 0 : _a5.classList.toggle("collapsed");
        });
      });
    }
  }
  ...
  replaceAISection(html3) {
    if (this.flashContentEl) {
      const existingAI = this.flashContentEl.querySelector(".ai-section");
      if (existingAI) {
        existingAI.outerHTML = html3;
      } else {
        this.flashContentEl.innerHTML += html3;
      }
    }
  }
  async renderAIContent(content, word, availableServices = [], currentServiceId = "") {
    if (!this.flashContentEl)
      return;
    let aiSection = this.flashContentEl.querySelector(".ai-section");
    if (!aiSection) {
      aiSection = this.flashContentEl.createDiv({ cls: "dictionary-section ai-section" });
    } else {
      aiSection.empty();
      aiSection.className = "dictionary-section ai-section";
    }
    const header = aiSection.createDiv({ cls: "dictionary-header" });
    const titleContainer = header.createDiv({ cls: "dictionary-title-container" });
    titleContainer.createSpan({ cls: "dictionary-title", text: "\u{1F916} AI" });
    if (availableServices.length > 1) {
      const switcher = titleContainer.createEl("select", { cls: "ai-service-switcher" });
      availableServices.forEach((service) => {
        const option = switcher.createEl("option", {
          text: service.name,
          value: service.id
        });
      ...
      switcher.addEventListener("change", (e) => {
        ...
        const contentDiv2 = aiSection.querySelector(".ai-content");
        if (contentDiv2) {
          contentDiv2.innerHTML = '<div class="ai-loading">\u6B63\u5728\u5207\u6362 AI \u6A21\u578B...</div>';
        }
        document.dispatchEvent(new CustomEvent("switch-ai-service", {
          detail: { word, serviceId: selectedId }
        }));
      });
    }
    const actionsDiv = header.createDiv({ cls: "dictionary-actions" });
    const saveBtn = actionsDiv.createEl("button", {
      cls: "ai-save-btn",
      text: "\u{1F9E0}"
    });
    saveBtn.title = "\u4FDD\u5B58\u5230 Flashcard";
    saveBtn.addEventListener("click", () => {
      this.saveToFlashcard(word, content);
    });
    const contentDiv = aiSection.createDiv({ cls: "dictionary-content ai-content" });
    contentDiv.setAttribute("title", "\u53CC\u51FB\u53EF\u7F16\u8F91\u5185\u5BB9");
    await this.renderMarkdownContent(content, contentDiv);
    contentDiv.addEventListener("dblclick", (e) => {
      if (e.target.tagName === "A" || e.target.tagName === "BUTTON")
        return;
      this.activateEditMode(contentDiv, content, word);
    });
  }
  setAILoading() {
    if (!this.flashContentEl)
      return false;
    const aiSection = this.flashContentEl.querySelector(".ai-section");
    if (aiSection) {
      const contentDiv = aiSection.querySelector(".ai-content");
      if (contentDiv) {
        contentDiv.innerHTML = '<div class="ai-loading">\u6B63\u5728\u83B7\u53D6...</div>';
        return true;
      }
    }
    return false;
  }
  async renderMarkdownContent(content, container) {
    container.empty();
    ...
    await import_obsidian10.MarkdownRenderer.render(
      ...
      "",
      null
      // Component reference not needed if we don't use internal links excessively or if we manage it otherwise
    );
  }
  activateEditMode(container, currentContent, word) {
    const height = container.offsetHeight;
    container.empty();
    container.addClass("is-editing");
    const textarea = container.createEl("textarea", { cls: "ai-edit-textarea" });
    textarea.value = currentContent;
    textarea.style.height = `${Math.max(100, height)}px`;
    const btnContainer = container.createDiv({ cls: "ai-edit-actions" });
    const saveBtn = btnContainer.createEl("button", { text: "\u4FDD\u5B58", cls: "mod-cta" });
    const cancelBtn = btnContainer.createEl("button", { text: "\u53D6\u6D88" });
    saveBtn.addEventListener("click", async () => {
      const newContent = textarea.value;
      container.removeClass("is-editing");
      await this.renderMarkdownContent(newContent, container);
      container.setAttribute("data-content", newContent);
    });
    cancelBtn.addEventListener("click", async () => {
      container.removeClass("is-editing");
      await this.renderMarkdownContent(currentContent, container);
    });
  }
  setWord(word) {
  ...
  triggerLookup(word) {
    this.currentWord = word;
    document.dispatchEvent(new CustomEvent("flash-lookup", { detail: { word } }));
  }
  async saveToFlashcard(word, content) {
    document.dispatchEvent(new CustomEvent("save-ai-to-flashcard", { detail: { word, content } }));
  }
  copyContent() {
    if (this.flashContentEl) {
      const text3 = this.flashContentEl.innerText;
      navigator.clipboard.writeText(text3).then(() => {
        if (this.containerEl) {
          const btn = this.containerEl.querySelector('[title="\u590D\u5236\u7ED3\u679C"]');
          if (btn) {
            const original = btn.textContent;
            btn.textContent = "\u2713";
            setTimeout(() => btn.textContent = original, 1e3);
          }
        }
      });
    }
...
// History components removed

// src/ai/ai-service.ts
var import_obsidian16 = require("obsidian");
async function queryAI(word, config) {
  if (!config.enabled || !config.apiKey) {
    return { content: "", error: "AI not configured" };
  }
  const prompt = config.prompt.replace(/\{word\}/g, word);
  try {
    if (config.provider === "openai" || config.provider === "generic") {
      return await queryOpenAI(prompt, config);
    } else if (config.provider === "anthropic") {
      return await queryAnthropic(prompt, config);
    }
    return { content: "", error: "Unknown provider" };
  } catch (error) {
    console.error("AI query failed:", error);
    return { content: "", error: error.message || "AI query failed" };
  }
}
async function queryOpenAI(prompt, config) {
  ...
  const data2 = response.json;
  if (data2.error) {
    return { content: "", error: data2.error.message };
  }
  return { content: ((_c = (_b = (_a5 = data2.choices) == null ? void 0 : _a5[0]) == null ? void 0 : _b.message) == null ? void 0 : _c.content) || "" };
}
async function queryAnthropic(prompt, config) {
  ...
  return { content: ((_b = (_a5 = data2.content) == null ? void 0 : _a5[0]) == null ? void 0 : _b.text) || "" };
}
// src/vocabulary/history.ts
// LookupHistory class removed

// src/ai/document-analyzer.ts
async function analyzeDocument(content, config) {
  ...
  if (!config.enabled || !config.apiKey) {
    return {
      success: false,
      error: "AI\u672A\u914D\u7F6E\uFF0C\u8BF7\u5728\u8BBE\u7F6E\u4E2D\u914D\u7F6EAPI\u5BC6\u94A5"
    };
  }
  const prompt = `\u5206\u6790\u4EE5\u4E0B\u6587\u6863\u5185\u5BB9\uFF0C\u63D0\u53D6\u7ED3\u6784\u5316\u5143\u6570\u636E\u3002

\u8981\u6C42\uFF1A
1. tags: 3-5\u4E2A\u76F8\u5173\u6807\u7B7E\uFF08\u82F1\u6587\uFF0C\u5C0F\u5199\uFF0C\u7528\u8FDE\u5B57\u7B26\u8FDE\u63A5\uFF0C\u5982\uFF1Aprogramming, note-taking, productivity\uFF09
2. summary: \u75281-2\u53E5\u8BDD\u7B80\u6D01\u603B\u7ED3\u6587\u6863\u4E3B\u8981\u5185\u5BB9\uFF08\u4E2D\u6587\uFF09
3. categories: 1-2\u4E2A\u6587\u6863\u6240\u5C5E\u5206\u7C7B\uFF08\u4E2D\u6587\uFF0C\u5982\uFF1A\u6280\u672F\u6587\u6863, \u5B66\u4E60\u7B14\u8BB0\uFF09
4. keywords: 3-5\u4E2A\u5173\u952E\u8BCD\uFF08\u4E2D\u6587\uFF0C\u5982\uFF1AObsidian, \u63D2\u4EF6\u5F00\u53D1, TypeScript\uFF09
5. status: \u6587\u6863\u72B6\u6001\uFF0C\u53EF\u9009\u503C\uFF1A\u8349\u7A3F/\u8FDB\u884C\u4E2D/\u5DF2\u5B8C\u6210/\u5F52\u6863
6. priority: \u4F18\u5148\u7EA7\uFF0C\u53EF\u9009\u503C\uFF1A\u4F4E/\u4E2D/\u9AD8
...
${content.substring(0, 3e3)}`;
  try {
    ...
    const response = await queryAI("", analysisConfig);
    if (response.error) {
      return {
        success: false,
        error: `AI\u5206\u6790\u5931\u8D25: ${response.error}`
      };
    }
    if (!response.content) {
      return {
    ...
    const metadata = parseAIResponse(response.content);
    return {
      success: true,
      metadata
    };
...
function parseFrontmatter(content) {
  ...
  if (match) {
    try {
      const yaml = (0, import_obsidian17.parseYaml)(match[1]);
      return {
        yaml: yaml || {},
        body: match[2]
      };
...
function mergeMetadata(existing, newMetadata) {
  var _a5;
  const merged = { ...existing };
  const existingTags = Array.isArray(merged.tags) ? merged.tags : [];
  const allTags = [.../* @__PURE__ */ new Set([...existingTags, ...newMetadata.tags])];
  if (allTags.length > 0) {
    merged.tags = allTags;
  }
  if (!merged.summary || newMetadata.summary.length > (((_a5 = merged.summary) == null ? void 0 : _a5.length) || 0)) {
    merged.summary = newMetadata.summary;
  }
  const existingCategories = Array.isArray(merged.categories) ? merged.categories : [];
  const allCategories = [.../* @__PURE__ */ new Set([...existingCategories, ...newMetadata.categories])];
  if (allCategories.length > 0) {
    merged.categories = allCategories;
  }
  const existingKeywords = Array.isArray(merged.keywords) ? merged.keywords : [];
  const allKeywords = [.../* @__PURE__ */ new Set([...existingKeywords, ...newMetadata.keywords])];
  if (allKeywords.length > 0) {
    merged.keywords = allKeywords;
  }
  if (newMetadata.status) {
    merged.status = newMetadata.status;
  }
  if (newMetadata.priority) {
    merged.priority = newMetadata.priority;
  }
  return merged;
}
function formatFrontmatter(metadata) {
  ...
  const yamlString = (0, import_obsidian17.stringifyYaml)(metadata);
  return `---
${yamlString.trim()}
---`;
}
function insertOrUpdateFrontmatter(content, metadata) {
  if (!metadata || Object.keys(metadata).length === 0) {
    return content;
  }
  const yamlString = formatFrontmatter(metadata);
...
// src/main.ts
var InputModal = class extends import_obsidian18.Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl("h3", { text: "Enter word to lookup" });
    ...
    const submitBtn = contentEl.createEl("button", { text: "Lookup" });
...
// ── Query View ────────────────────────────────────────────────────
var import_obsidian_qv = require("obsidian");
var QUERY_VIEW_TYPE = "word-query-view";
var QuerySidebarView = class extends import_obsidian_qv.ItemView {
  constructor(leaf, settings, plugin) {
    super(leaf);
    this.activeTab = "lookup";
    this.settings = settings;
    this.plugin = plugin;
    this.flashLookupComponent = new FlashLookupComponent(this.app);
  }
  getViewType() { return QUERY_VIEW_TYPE; }
  getDisplayText() { return "Word Query"; }
  ...
  switchTab(tabId) {
    ...
    switch (tabId) {
      case "lookup":  this.flashLookupComponent.mount(this.contentContainer); break;
    }
  }
};

// ── Query Settings Tab ────────────────────────────────────────────
var import_obsidian_qs = require("obsidian");
var QuerySettingTab = class extends import_obsidian_qs.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    containerEl.createEl("h3", { text: "\u4e2d\u6587\u8bcd\u5178" });
    new import_obsidian_qs.Setting(containerEl).setName("\u6c49\u5178 (Zdic)").addToggle(
      (t) => t.setValue(this.plugin.settings.chineseDictionaries.zdic)
        .onChange(async (v) => { this.plugin.settings.chineseDictionaries.zdic = v; await this.plugin.saveSettings(); })
    );
    new import_obsidian_qs.Setting(containerEl).setName("\u641c\u97f5 (Sou-Yun)").addToggle(
      (t) => t.setValue(this.plugin.settings.chineseDictionaries.souyun)
        .onChange(async (v) => { this.plugin.settings.chineseDictionaries.souyun = v; await this.plugin.saveSettings(); })
    );
    containerEl.createEl("h3", { text: "\u82f1\u6587\u8bcd\u5178" });
    new import_obsidian_qs.Setting(containerEl).setName("\u6709\u9053 (Youdao)").addToggle(
      (t) => t.setValue(this.plugin.settings.englishDictionaries.youdao)
        .onChange(async (v) => { this.plugin.settings.englishDictionaries.youdao = v; await this.plugin.saveSettings(); })
    );
    new import_obsidian_qs.Setting(containerEl).setName("\u6717\u6587 (Longman)").addToggle(
      (t) => t.setValue(this.plugin.settings.englishDictionaries.longman)
        .onChange(async (v) => { this.plugin.settings.englishDictionaries.longman = v; await this.plugin.saveSettings(); })
    );
    containerEl.createEl("h2", { text: "\u57fa\u672c\u8bbe\u7f6e" });
    new import_obsidian_qs.Setting(containerEl).setName("\u89e6\u53d1\u65b9\u5f0f").addDropdown(
      (dd) => dd.addOption("altKey", "Alt + \u9009\u4e2d").addOption("ctrlKey", "Ctrl + \u9009\u4e2d")
        .addOption("shiftKey", "Shift + \u9009\u4e2d").addOption("disable", "\u7981\u7528")
        .setValue(this.plugin.settings.functionKey)
        .onChange(async (v) => { this.plugin.settings.functionKey = v; await this.plugin.saveSettings(); })
    );
    containerEl.createEl("h2", { text: "\u6536\u85cf\u8bbe\u7f6e" });
    new import_obsidian_qs.Setting(containerEl).setName("AI \u7B14\u8BB0\u8DEF\u5F84").setDesc("AI \u6536\u85CF\u5185\u5BB9\u4FDD\u5B58\u7684\u6587\u4EF6\u8DEF\u5F84").addText(
      (t) => t.setPlaceholder(".flashcards/words.json").setValue(this.plugin.settings.collectionSettings.aiNotePath)
        .onChange(async (v) => { this.plugin.settings.collectionSettings.aiNotePath = v; await this.plugin.saveSettings(); })
    );

    containerEl.createEl("h2", { text: "\u5386\u53f2\u8bbe\u7f6e" });
    new import_obsidian_qs.Setting(containerEl).setName("\u542f\u7528\u67e5\u8be2\u5386\u53f2").addToggle(
      (t) => t.setValue(this.plugin.settings.historySettings.enabled).onChange(async (v) => {
        this.plugin.settings.historySettings.enabled = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian_qs.Setting(containerEl).setName("\u5386\u53f2\u6587\u4ef6\u8def\u5f84").addText(
      (t) => t.setValue(this.plugin.settings.historySettings.historyPath).setPlaceholder(".flashcards/history.json").onChange(async (v) => {
        this.plugin.settings.historySettings.historyPath = v;
        await this.plugin.saveSettings();
      })
    );
    // AI Services
    const aiDetails = containerEl.createEl("details");
    aiDetails.createEl("summary", { text: "AI \u8bbe\u7f6e" });
    new import_obsidian_qs.Setting(aiDetails).setName("\u542f\u7528 AI \u89e3\u91ca").addToggle(
      (t) => t.setValue(this.plugin.settings.showAI)
        .onChange(async (v) => { this.plugin.settings.showAI = v; await this.plugin.saveSettings(); })
    );
    this.plugin.settings.aiServices.forEach((service, idx) => {
      const div = aiDetails.createDiv();
      div.style.cssText = "margin-bottom:12px;padding:10px;background:var(--background-secondary);border-radius:5px;";
      new import_obsidian_qs.Setting(div).setName("\u542f\u7528").addToggle(
        (t) => t.setValue(service.enabled).onChange(async (v) => { this.plugin.settings.aiServices[idx].enabled = v; await this.plugin.saveSettings(); })
      );
      new import_obsidian_qs.Setting(div).setName("\u540d\u79f0").addText(
        (t) => t.setValue(service.name).onChange(async (v) => { this.plugin.settings.aiServices[idx].name = v; await this.plugin.saveSettings(); })
      );
      new import_obsidian_qs.Setting(div).setName("API Key").addText(
        (t) => t.setPlaceholder("sk-...").setValue(service.apiKey).onChange(async (v) => { this.plugin.settings.aiServices[idx].apiKey = v; await this.plugin.saveSettings(); })
      );
      new import_obsidian_qs.Setting(div).setName("Base URL").addText(
        (t) => t.setValue(service.baseURL).onChange(async (v) => { this.plugin.settings.aiServices[idx].baseURL = v; await this.plugin.saveSettings(); })
      );
      new import_obsidian_qs.Setting(div).setName("\u6a21\u578b").addText(
        (t) => t.setValue(service.model).onChange(async (v) => { this.plugin.settings.aiServices[idx].model = v; await this.plugin.saveSettings(); })
      );
    });
    new import_obsidian_qs.Setting(aiDetails).addButton(
      (btn) => btn.setButtonText("\u2795 \u6dfb\u52a0 AI \u670d\u52a1").onClick(async () => {
        this.plugin.settings.aiServices.push({
          id: Date.now().toString(), name: "New AI Service", enabled: false,
          provider: "openai", apiKey: "", baseURL: "https://api.openai.com/v1",
          model: "gpt-3.5-turbo", prompt: '\u8bf7\u7b80\u6d01\u89e3\u91ca"{word}"\u8fd9\u4e2a\u8bcd\u7684\u542b\u4e49\uff0c100\u5b57\u4ee5\u5185\u3002'
        });
        await this.plugin.saveSettings();
        this.display();
      })
    ...
    // Quick Links
    const qlDetails = containerEl.createEl("details");
    qlDetails.createEl("summary", { text: "\u{1f517} \u5feb\u901f\u94fe\u63a5" });
    const qlList = qlDetails.createDiv();
    this.plugin.settings.quickLinks.forEach((link, idx) => {
      const row = qlList.createDiv({ attr: { style: "display:flex;align-items:center;gap:8px;margin-bottom:6px;" } });
      new import_obsidian_qs.Setting(row).addToggle(
        (t) => t.setValue(link.enabled).onChange(async (v) => { this.plugin.settings.quickLinks[idx].enabled = v; await this.plugin.saveSettings(); })
      );
      const nameInput = document.createElement("input");
      nameInput.type = "text"; nameInput.value = link.name; nameInput.style.width = "80px";
      nameInput.onchange = async () => { this.plugin.settings.quickLinks[idx].name = nameInput.value; await this.plugin.saveSettings(); };
      row.appendChild(nameInput);
      const urlInput = document.createElement("input");
      urlInput.type = "text"; urlInput.value = link.url; urlInput.style.flex = "1";
      urlInput.onchange = async () => { this.plugin.settings.quickLinks[idx].url = urlInput.value; await this.plugin.saveSettings(); };
...
// ── QueryPlugin ───────────────────────────────────────────────────
var import_obsidian_q = require("obsidian");
var QueryPlugin = class extends import_obsidian_q.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    // Migrate quickLinks from object to array if needed
    if (!Array.isArray(this.settings.quickLinks)) {
      const oldLinks = this.settings.quickLinks;
      this.settings.quickLinks = DEFAULT_SETTINGS.quickLinks.map((defaultLink) => {
        const oldKey = defaultLink.id;
        return oldLinks && typeof oldLinks[oldKey] !== "undefined"
          ? { ...defaultLink, enabled: oldLinks[oldKey] }
          : defaultLink;
      });
      await this.saveSettings();
    }
    // Migrate aiSettings → aiServices
    if (!this.settings.aiServices || this.settings.aiSettings) {
      const oldAI = this.settings.aiSettings;
      if (oldAI) {
        this.settings.aiServices = [{
          id: "migrated-default", name: "Default AI", enabled: oldAI.enabled,
          provider: oldAI.provider, apiKey: oldAI.apiKey, baseURL: oldAI.baseURL,
          model: oldAI.model, prompt: oldAI.prompt
        }];
        delete this.settings.aiSettings;
      } else if (!this.settings.aiServices) {
        this.settings.aiServices = JSON.parse(JSON.stringify(DEFAULT_SETTINGS.aiServices));
      }
      await this.saveSettings();
    }
    this.collection = new WordCollection(this.app, this.settings.collectionSettings);
    this.registerDomEvent(document, "collect-md",  () => { this.collectToMD(); });
    this.registerDomEvent(document, "copy-text", (e) => {
      if (e.detail && e.detail.text) {
        navigator.clipboard.writeText(e.detail.text);
        new import_obsidian_q.Notice("\u2705 \u5DF2\u590D\u5236\u5230\u526A\u8D34\u677F");
      }
    });
    this.registerDomEvent(document, "collect-ai",  () => { this.collectAIToMD(); });
    this.registerDomEvent(document, "save-ai-to-flashcard", (e) => {
      if (e.detail && e.detail.word && e.detail.content) {
        this.saveAIToFlashcard(e.detail.word, e.detail.content);
      }
    });
    this.registerDomEvent(document, "flash-lookup", (e) => {
      if (e.detail && e.detail.word) this.lookupWord(e.detail.word);
    });
    this.registerDomEvent(document, "switch-ai-service", (e) => {
      if (e.detail && e.detail.word && typeof e.detail.serviceId !== "undefined")
        this.queryAIExplanation(e.detail.word, e.detail.serviceId);
    });
    this.registerView(QUERY_VIEW_TYPE, (leaf) => new QuerySidebarView(leaf, this.settings, this));
    this.addRibbonIcon("search", "Word Query", () => { this.activateView("lookup"); });
    this.addCommand({
      id: "lookup-input-word", name: "Lookup word (input)",
      callback: () => { new InputModal(this.app, (word) => { this.lookupWord(word); }).open(); }
    });
    this.addCommand({
      id: "lookup-selected-word", name: "Lookup selected word",
      callback: () => {
        var _a;
        const activeView = this.app.workspace.getActiveViewOfType(import_obsidian_q.MarkdownView);
        if (activeView) {
          const sel = activeView.editor.getSelection().trim();
          if (sel) { activeView.editor.replaceSelection(`==${sel}==`); this.lookupWord(sel); }
        } else {
          const sel = (_a = window.getSelection()) == null ? void 0 : _a.toString().trim();
          if (sel) this.lookupWord(sel);
        }
      }
    });
    this.registerContextMenu();
    this.registerMouseEvents();
    this.addSettingTab(new QuerySettingTab(this.app, this));
    console.log("My Query plugin loaded");
  }
  initializeDictionary() {
    this.dictionary = DictionaryFactory.create(this.settings.primaryDictionary);
  }
  onunload() {
  ...
  async activateView(tabId) {
    var _a;
    const existing = this.app.workspace.getLeavesOfType(QUERY_VIEW_TYPE);
    if (existing.length === 0)
      await ((_a = this.app.workspace.getRightLeaf(false)) == null ? void 0 : _a.setViewState({ type: QUERY_VIEW_TYPE, active: true }));
    const leaves = this.app.workspace.getLeavesOfType(QUERY_VIEW_TYPE);
    if (leaves.length > 0) {
      this.app.workspace.revealLeaf(leaves[0]);
      const view = leaves[0].view;
      if (view && tabId) view.switchTab(tabId);
    }
  }
  async showFlashPanel() { await this.activateView("lookup"); }
  updateDisplayContent(html) {
    const leaves = this.app.workspace.getLeavesOfType(QUERY_VIEW_TYPE);
    if (leaves.length > 0) {
      const view = leaves[0].view;
      if (view && view.flashLookupComponent) {
        view.flashLookupComponent.setContent(html);
        if (html.includes("word-lookup-loading")) view.switchTab("lookup");
      }
    }
  }
  updateDisplayKeyword(word) {
    const leaves = this.app.workspace.getLeavesOfType(QUERY_VIEW_TYPE);
    if (leaves.length > 0) {
      const view = leaves[0].view;
      if (view && view.flashLookupComponent) view.flashLookupComponent.setWord(word);
    }
  }
  appendDisplayContent(html) {
    const leaves = this.app.workspace.getLeavesOfType(QUERY_VIEW_TYPE);
    if (leaves.length > 0) {
      const view = leaves[0].view;
      if (view && view.flashLookupComponent) view.flashLookupComponent.appendContent(html);
    }
  }
  updateDisplayAI(html) {
    const leaves = this.app.workspace.getLeavesOfType(QUERY_VIEW_TYPE);
    if (leaves.length > 0) {
      const view = leaves[0].view;
      if (view && view.flashLookupComponent) view.flashLookupComponent.replaceAISection(html);
    }
  }
  registerContextMenu() {
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, editor) => {
        ...
        if (selection) {
          menu.addItem((item) => {
            item.setTitle("Lookup: " + selection.substring(0, 20) + (selection.length > 20 ? "..." : ""))
              .setIcon("book-open")
              .onClick(() => { editor.replaceSelection(`==${selection}==`); this.lookupWord(selection); });
          });
        }
      })
    );
  ...
  registerMouseEvents() {
    this.registerDomEvent(document.body, "pointerup", (evt) => {
      var _a;
      const funcKey = this.settings.functionKey;
      if (funcKey === "disable") return;
      if (!evt[funcKey]) return;
      const target = evt.target;
      if (target.closest(".word-lookup-popup")) return;
      const activeView = this.app.workspace.getActiveViewOfType(import_obsidian_q.MarkdownView);
      let selection = "";
      if (activeView) {
        ...
        if (selection) {
          ...
          this.lookupWord(selection, { x: evt.pageX, y: evt.pageY });
          return;
        }
      }
      const rawSel = (_a = window.getSelection()) == null ? void 0 : _a.toString();
      if (rawSel) selection = rawSel.trim();
      if (selection) { evt.stopImmediatePropagation(); this.lookupWord(selection, { x: evt.pageX, y: evt.pageY }); }
    });
  }
  detectChinese(text) { return /[\u4e00-\u9fff\u3400-\u4dbf]/.test(text); }
  isSentence(text) {
    const trimmed = text.trim();
    return (/[.?!\u3002\uff1f\uff01]+$/.test(trimmed) || /\.{3,}$/.test(trimmed) || /\u2026+$/.test(trimmed)) && trimmed.length > 1;
  }
  getEnabledDictionaries(isChinese) {
    const dicts = [];
    if (isChinese) {
      if (this.settings.chineseDictionaries.zdic) dicts.push("zdic");
      if (this.settings.chineseDictionaries.souyun) dicts.push("souyun");
    } else {
      if (this.settings.englishDictionaries.youdao) dicts.push("youdao");
      if (this.settings.englishDictionaries.longman) dicts.push("longman");
    }
    return dicts;
  }
  combineResults(results, isChinese, word) {
    const dictNames = { zdic: "\u6c49\u5178", souyun: "\u641c\u97f5", youdao: "\u6709\u9053", longman: "\u6717\u6587" };
    let html = this.renderQuickLinks(word);
    for (const r of results) {
      const contentHtml = r.html && r.html.trim() !== "" ? r.html : '<div class="dictionary-empty">\u672a\u627e\u5230\u91ca\u4e49</div>';
      html += `<div class="dictionary-section">
        <div class="dictionary-header">
          <span class="dictionary-title">${dictNames[r.type] || r.type}</span>
          <div class="dictionary-actions">
            <button class="ai-collect-btn" onclick="const content=this.closest('.dictionary-section').querySelector('.dictionary-content').innerText; const word=document.querySelector('.word-title')?.innerText || ''; document.dispatchEvent(new CustomEvent('save-ai-to-flashcard', {detail:{word:word, content:content}}))" title="\u4FDD\u5B58\u5230 Flashcard">\u{1F9E0} \u4FDD\u5B58</button>
          </div>
        </div>
        <div class="dictionary-content">${contentHtml}</div>
      </div>`;
    }
    return html;
  }
  renderQuickLinks(word) {
    const quickLinks = this.settings.quickLinks;
    if (!Array.isArray(quickLinks)) return "";
    const enabledLinks = quickLinks.filter((l) => l.enabled);
    if (enabledLinks.length === 0) return "";
    let html = '<div class="quick-links"><div class="quick-links-header"><span class="quick-links-label">\u{1f517} \u5feb\u901f\u94fe\u63a5</span><span class="quick-links-toggle">\u25bc</span></div><div class="quick-links-buttons">';
    enabledLinks.forEach((link) => {
      const finalUrl = link.url.replace("{word}", encodeURIComponent(word));
      html += `<a href="${finalUrl}" class="quick-link-btn" target="_blank">${link.icon || "\u{1f517}"} ${link.name}</a>`;
    });
    html += "</div></div>";
    return html;
  }
  ...
  async lookupWord(word, position) {
    if (!word) return;
    const isChinese = this.detectChinese(word);
    const isSentence = this.isSentence(word);
    try {
      await this.showFlashPanel();
      this.updateDisplayContent('<div class="word-lookup-loading">Loading...</div>');
      this.updateDisplayKeyword(word);
      if (isSentence) {
        let html = this.renderQuickLinks(word);
        html += '<div class="sentence-notice">\u{1f4dd} \u68c0\u6d4b\u5230\u53e5\u5b50\uff0c\u8df3\u8fc7\u8bcd\u5178\u67e5\u8be2\uff0c\u4ec5\u663e\u793aAI\u89e3\u91ca</div>';
        this.updateDisplayContent(html);
        this.currentResult = { word, basicTranslation: [] };
        this.saveToHistory(word, null);
      } else {
        const dictionaryTypes = this.getEnabledDictionaries(isChinese);
        const allResults = [];
        await Promise.all(dictionaryTypes.map(async (dictType) => {
          try {
            const dict = DictionaryFactory.create(dictType);
            const result = await dict.lookup(word);
            const html = dict.formatResult(result, word, this.settings);
            allResults.push({ type: dictType, result, html });
          } catch (err) { console.error(`Dictionary ${dictType} lookup failed:`, err); }
        }));
        allResults.sort((a, b) => dictionaryTypes.indexOf(a.type) - dictionaryTypes.indexOf(b.type));
        if (allResults.length > 0) {
          this.currentResult = allResults[0].result;
        } else {
          this.currentResult = { word, basicTranslation: [] };
        }
        this.updateDisplayContent(this.combineResults(allResults, isChinese, word));
        this.saveToHistory(word, this.currentResult);
        const noDictResults = allResults.length === 0;
        if (noDictResults) {
          this.appendDisplayContent('<div class="ai-fallback-notice">\u26a0\ufe0f \u672a\u627e\u5230\u8bcd\u5178\u7ed3\u679c\uff0c\u81ea\u52a8\u8c03\u7528 AI \u89e3\u91ca...</div>');
        }
        if ((this.settings.showAI || noDictResults) && this.settings.aiServices.some((s) => s.enabled))
          this.queryAIExplanation(word);
      }
      if (!isSentence && this.settings.autoPronunciation && ((_d = this.currentResult) == null ? void 0 : _d.word))
        this.playPronunciation(this.currentResult.word);
    } catch (error) {
      console.error("Lookup failed:", error);
      this.updateDisplayContent(`<div class="word-lookup-error">Failed to lookup: ${word}</div>`);
    }
  }
  async queryAIExplanation(word, serviceId) {
    let aiService;
    if (serviceId) {
      aiService = this.settings.aiServices.find((s) => s.id === serviceId);
    } else {
      aiService = this.settings.aiServices.find((s) => s.enabled);
    }
    if (!aiService || !aiService.apiKey) return;
    this.currentAIServiceId = aiService.id;
    let updated = false;
    const leaves = this.app.workspace.getLeavesOfType(QUERY_VIEW_TYPE);
    if (leaves.length > 0) {
      const view = leaves[0].view;
      if (view && view.flashLookupComponent && typeof view.flashLookupComponent.setAILoading === "function")
        updated = view.flashLookupComponent.setAILoading();
    }
    if (!updated)
      this.appendDisplayContent('<div class="ai-section"><div class="ai-title">\u{1f916} AI</div><div class="ai-loading">\u6b63\u5728\u83b7\u53d6...</div></div>');
    try {
      const response = await queryAI(word, {
        enabled: true, provider: aiService.provider, apiKey: aiService.apiKey,
        baseURL: aiService.baseURL, model: aiService.model, prompt: aiService.prompt
      });
      if (response.error) {
        this.updateDisplayAI(`<div class="dictionary-section ai-section"><div class="dictionary-header"><span class="dictionary-title">\u{1f916} AI</span></div><div class="dictionary-content ai-content ai-error">\u9519\u8bef: ${response.error}</div></div>`);
      } else if (response.content) {
        this.currentAIContent = response.content;
        const leaves2 = this.app.workspace.getLeavesOfType(QUERY_VIEW_TYPE);
        if (leaves2.length > 0) {
          const view = leaves2[0].view;
          if (view && view.flashLookupComponent && typeof view.flashLookupComponent.renderAIContent === "function") {
            const availableServices = this.settings.aiServices.map((s) => ({ id: s.id, name: s.name, enabled: s.enabled }));
            await view.flashLookupComponent.renderAIContent(response.content, word, availableServices, aiService.id);
          }
        }
      }
    } catch (error) {
      this.updateDisplayAI(`<div class="dictionary-section ai-section"><div class="dictionary-header"><span class="dictionary-title">\u{1f916} AI</span></div><div class="dictionary-content ai-content ai-error">\u9519\u8bef: ${error.message || "Unknown error"}</div></div>`);
    }
  }
  collectToMD() {
    if (this.currentResult) {
      this.collection.saveWord(this.currentResult);
      new import_obsidian_q.Notice("\u{1f4dd} \u5df2\u6536\u85cf\u5230MD\u6587\u4ef6");
    } else {
      new import_obsidian_q.Notice("\u6ca1\u6709\u53ef\u6536\u85cf\u7684\u5185\u5bb9");
    }
  }
  async collectAIToMD() {
    var _a;
    if (this.currentAIContent && ((_a = this.currentResult) == null ? void 0 : _a.word)) {
      const activeService = this.settings.aiServices.find((s) => s.id === this.currentAIServiceId);
      let template = activeService == null ? void 0 : activeService.exportTemplate;
      if (!template) template = "## {word}\n\n{content}\n\n---\n";
      let cleanContent = this.currentAIContent;
      const wordStr = this.currentResult.word.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      cleanContent = cleanContent.replace(new RegExp(`^##?\\s*${wordStr}\\s*\\n`, "i"), "");
      cleanContent = cleanContent.replace(/^(AI\u89e3\u91ca[::\uff1a]?\s*\n?)/i, "").trim();
      const content = template.replace(/{word}/g, this.currentResult.word).replace(/{content}/g, cleanContent);
      try {
        const filePath = (activeService == null ? void 0 : activeService.exportPath) || this.settings.collectionSettings.aiNotePath || "sources/vocabulary/gpt-notes.md";
        const folderPath = filePath.substring(0, filePath.lastIndexOf("/"));
        if (folderPath && !this.app.vault.getAbstractFileByPath(folderPath))
          await this.app.vault.createFolder(folderPath);
        let file = this.app.vault.getAbstractFileByPath(filePath);
        if (!file) { await this.app.vault.create(filePath, content); }
        else { const existing = await this.app.vault.read(file); await this.app.vault.modify(file, existing + "\n" + content); }
        new import_obsidian_q.Notice(`\u{1f916} AI\u7ed3\u679c\u5df2\u4fdd\u5b58\u81f3 ${filePath}`);
      } catch (error) {
        new import_obsidian_q.Notice("\u4fdd\u5b58\u5931\u8d25: " + error.message);
      }
    } else {
      new import_obsidian_q.Notice("\u6ca1\u6709AI\u5185\u5bb9\u53ef\u6536\u85cf");
    }
  }
  async saveAIToFlashcard(word, aiContent) {
    const flashcardPath = this.settings.collectionSettings.flashcardPath || ".flashcards/words.json";
    try {
      const adapter = this.app.vault.adapter;
      let flashcards = [];
      if (await adapter.exists(flashcardPath)) {
        const fileContent = await adapter.read(flashcardPath);
        try { flashcards = JSON.parse(fileContent); } catch (e) { console.error("Parse flashcards.json failed", e); }
      }
      const existingIdx = flashcards.findIndex((f) => f.front === word);
      const now = Date.now();
      const newCard = {
        id: `ai-${word}-${now}`,
        front: word,
      ...
      if (existingIdx >= 0) { flashcards[existingIdx] = newCard; }
      else { flashcards.push(newCard); }
      const folderPath = flashcardPath.substring(0, flashcardPath.lastIndexOf("/"));
      if (folderPath && !await adapter.exists(folderPath)) { await adapter.mkdir(folderPath); }
      await adapter.write(flashcardPath, JSON.stringify(flashcards, null, 2));
      new import_obsidian_q.Notice(`\u2705 \u5df2\u4fdd\u5b58 AI Flashcard \u81f3 ${flashcardPath}`);
    } catch (err) {
      new import_obsidian_q.Notice(`\u274c \u4fdd\u5b58 Flashcard \u5931\u8d25: ${err.message}`);
    }
  }
  async saveToHistory(word, dictResult) {
    if (!word || !this.settings.historySettings.enabled) return;
    const historyPath = this.settings.historySettings.historyPath || ".flashcards/history.json";
    try {
      const adapter = this.app.vault.adapter;
      let history = [];
      if (await adapter.exists(historyPath)) {
        const content = await adapter.read(historyPath);
        try { history = JSON.parse(content); } catch (e) { console.error("Parse history.json failed", e); }
      }
      
      let definition = "";
      if (dictResult && dictResult.basicTranslation) {
        definition = Array.isArray(dictResult.basicTranslation) ? dictResult.basicTranslation.join("; ") : dictResult.basicTranslation;
      }
      
      const now = Date.now();
      const entryId = `history-${word.toLowerCase().replace(/[^a-z0-9]/g, "-")}-${now}`;
      
      const existingIdx = history.findIndex((h) => h.front === word);
      let entry;
      
      if (existingIdx >= 0) {
        entry = history[existingIdx];
        if (definition) entry.back = definition;
      ...
      const folderPath = historyPath.substring(0, historyPath.lastIndexOf("/"));
      if (folderPath && !await adapter.exists(folderPath)) { await adapter.mkdir(folderPath); }
      await adapter.write(historyPath, JSON.stringify(history, null, 2));
    } catch (err) {
  ...
  async saveSettings() {
    ...
    this.collection.updateSettings(this.settings.collectionSettings);
  }
};
